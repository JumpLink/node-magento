...
module.exports = function(config, isSecure) {
	var xmlrpc = require('xmlrpc');
	
	var createClientFunction = xmlrpc.createClient;
	if(isSecure) {
		createClientFunction = xmlrpc.createSecureClient;
	}
	...
	return xmlrpc = {
		...
		client: createClientFunction(config),
					...
					if (!err) {
						for (var j =0; j < resources.length; j++) {
							...
							for (var i = 0; i < methods.length; i++) {
								...
								console.log("define "+method.path);
